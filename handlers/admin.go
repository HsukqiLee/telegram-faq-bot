...
func (h *AdminHandler) HandleSuperAdminCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message) {
	...
	case "listadmin":
		h.HandleListAdminCommand(bot, message, 0)
		return
	}
...
func (h *AdminHandler) HandleListAdminCommand(bot *tgbotapi.BotAPI, message *tgbotapi.Message, page int) {
	const pageSize = 5
	start := page * pageSize
	...
	if page > 0 {
		prevButton := tgbotapi.NewInlineKeyboardButtonData("上一页", fmt.Sprintf("listadmin_%d", page-1))
		navButtons = append(navButtons, prevButton)
	}
	if end < len(adminIDs) {
		nextButton := tgbotapi.NewInlineKeyboardButtonData("下一页", fmt.Sprintf("listadmin_%d", page+1))
	...
	msg := tgbotapi.NewMessage(message.Chat.ID, "选择一个管理员进行操作：")
